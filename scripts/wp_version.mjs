...
const getMajorVersionIndexFromData = (majorVersion, isRollingVersion) => versionData.versions.findIndex(
    (version) => version.name === majorVersion && version.isRolling === isRollingVersion,
);
const getMajorVersionNameFromMinor = (minorVersion) => minorVersion.split('.').slice(0, 2).join('.');
const updateMajorVersionWithData = (majorVersion, data, isRollingVersion = false) => {
    const index = getMajorVersionIndexFromData(majorVersion, isRollingVersion);
...
const getRollingVersion = async () => {
    const { isRolling } = await inquirer.prompt({
        type: 'confirm',
        name: 'isRolling',
        message: 'Is this a Rolling release?',
        default: false,
    });
...
const getImprovementsandfeaturesEndDate = async (releaseDate, isRolling) => {
    if (!isRolling) {
        return '';
    }
    const getSuggestedDate = () => {
        const supportLength = 4;
        let calculatedDate;
    ...
    const { theDate } = await inquirer.prompt({
        type: 'input',
        name: 'theDate',
        message: 'What is the improvements and new features end date?',
        default: getSuggestedDate(),
        validate: (input) => {
            const parsedDate = new Date(input);
...
const getReleaseVersionForMajorMinor = async (releaseDate) => {
...
const addMinorRelease = async (
    ...
    const isRollingVersion = await getRollingVersion();
    const thisVersionData = getMajorVersionFromData(majorVersionName, isRollingVersion);
    ...
    const nextReleaseVersion = await getReleaseVersionForMajorMinor(releaseDate);

    const schema = {
        name: nextReleaseName,
        releaseDate: getDateFormattedForVersionFile(releaseDate),
        version: Number(nextReleaseVersion),
...
const addReleaseMarkdown = async (name, isRolling, isLTS) => {
    let releaseName = `Moodle Workplace ${name}`;
    let isRollingRelease = '';
    if (isLTS) {
        releaseName = `${releaseName} (LTS)`;
    }
    if (isRolling) {
...
const addMajorRelease = async (
    ...
    const isRolling = await getRollingVersion();
    ...
    const improvementsandnewfeaturesEndDate = await getImprovementsandfeaturesEndDate(releaseDate, isRolling);
    ...
    const schema = {
        ...
        improvementsandnewfeaturesEndDate: improvementsandnewfeaturesEndDate
            ? getDateFormattedForVersionFile(improvementsandnewfeaturesEndDate)
    ...
    const releaseVersion = await getReleaseVersionForMajorMinor(releaseDate);
    ...
    console.log(schema);
    await persistVersionData();
    await addMinorRelease(!minors.length, releaseDate, minors);
};
