...
export const getTitle = (majorVersion: string, versionData: majorVersionData): string => {
    if (versionData.isLTS) {
        return `Moodle Workplace ${majorVersion} (LTS)`;
    }

    if (versionData.isRolling) {
        return `Moodle Workplace ${majorVersion} Rolling`;
    }
...
export const getVersionTitle = (versionName: string, isLTS: boolean = false, isRolling: boolean = false): string => {
    if (isLTS) {
        return `Moodle Workplace ${versionName} (LTS)`;
    }

    if (isRolling) {
        return `Moodle Workplace ${versionName} Rolling`;
    }

    return `Moodle Workplace ${versionName}`;
};

export const getVersionLabel = (versionName: string, isLTS: boolean = false, isRolling: boolean = false): string => {
    if (isLTS) {
        return `${versionName} (LTS)`;
    }

    if (isRolling) {
        return `${versionName} Rolling`;
    }
...
export const getVersion = (versionName: string, isRolling: boolean): majorVersionData => {
    const [major, release] = versionName.split('.');
    const majorVersion = `${major}.${release}`;
    return getAllVersions().find((version) => version.name === majorVersion && version.isRolling === isRolling);
};

export const getLatestRelease = (versionData: majorVersionData): string => {
    if (!versionData?.releases || versionData.releases.length === 0) {
        return '';
    }

    const latestRelease = versionData.releases.reduce((latest, current) => {
        if (new Date(current.releaseDate) > new Date(latest.releaseDate)) {
            return current;
        }
        return latest;
    }, versionData.releases[0]);

    return latestRelease.releaseDate;
};
