...
`version.php` must **never** contain `require_once` (or any other file inclusion). It is parsed in performance-sensitive bootstrap paths and must remain a plain data file with no side-effects.
