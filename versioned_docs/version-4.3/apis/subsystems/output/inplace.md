...
```
Of course, core/inplace_edit can also be used as a partial in another template.


## Render inplace_edit with JavaScript

```
$itemid = 153 // Id of the element to be modiefied inplace.

$renderer = $PAGE->get_renderer('core');
$inplaceedit = new tool_mytest\local\inplace_edit_text($record);
$params = $inplaceedit->export_for_template($renderer);
```
The params are transferred via webservice and are then processed by
```
