...
3. Populate the form with existing tags before calling $form->set_data($data):
 ...
 ```php
 $data = $DB->get_record('wiki_pages', array('id' => $this->page->id)); // Well, it's more complicated than that of course....
...
protected function define_structure() {
    ...
    // ...
    $glossary->add_child($tags); // Note that parent node is the glossary, not glossary entry, however the individual entries are tagged.
    $tags->add_child($tag);
    // ...
...
Custom plugins may go beyond the standard tags handling and use them without mixing with regular course/user/wiki/blogs tags, hide them from the "Tag search" page and "Tags" block but instead have their own interface to search/categorise using tags. This can be achieved by defining tag collection, make it not searchable and specify a custom URL to link to when the tag is actually displayed. This all can be defined in db/tag.php, see the comments in [lib/db/tag.php](https://github.com/moodle/moodle/blob/master/lib/db/tag.php)
