...
Scope requirements are read from the current route by the `\core\router\middleware\moodle_scope_attribute_middleware`, which attaches the resolved `scopeset[]` to the request as an attribute.
