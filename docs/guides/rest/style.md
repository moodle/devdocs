...
Nested resources MUST only be used for direct ownership or containment relationships.
...
/mod_forum/discussions{discussionId}/posts/{postId}
```
...
Declarative payloads describe what the resource should look like afterwards, which is easier to validate against a schema and safer to retry. Imperative payloads like "move": "up" encode a transition rather than a state, so retries or race conditions between two clients could produce different results depending on execution order, making PATCH retries unsafe (see section 15.2).
