...
If you only intend to run the app with the goal of executing Behat tests, you can use [the Docker images for the Mobile App](../docker-images). Again, make sure that you're running them on the testing environment using the `-test` suffix.
...
The Moodle App [only works in Chromium-based browsers](../app-in-browser), so mobile tests will be ignored if you are using any other browser. You can learn how to configure the browser used in your tests in the [[Running acceptance test]] page.
...
If you insert a pause into your test and open the developer tools, you can debug the application like you would during development. You can learn how to do that in [Using the Moodle App in a browser](../app-in-browser).
...
You can learn more about writing custom steps in the [[Writing new acceptance test step definitions]] page, and if you want to see how the steps that are specific to the app work, you should look into [behat_app.php](https://github.com/moodlehq/moodle-local_moodlemobileapp/blob/master/tests/behat/behat_app.php) and [app_behat_runtime.js](https://github.com/moodlehq/moodle-local_moodlemobileapp/blob/master/tests/behat/app_behat_runtime.js).
