...
- It is very desirable to begin translating the upcoming next (DEV) Moodle core version before the [expected release date](../../../../releases.md#general-release-calendar). You can always foresee that some Moodle users will want to start working in the newest Moodle version as soon as it is available; it would be better if is already translated by then.
